...
// Midrashon 1.0 - Female phrasing & Live Firestore for Midrashot
...
export default function App() {
  ...
  // Fetch Yeshivot list function (triggered on "התחלי" or when entering Admin/Results)
  const loadYeshivot = async () => {
    ...
    try {
      ...
      console.error("Error loading midrashot:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCalculateMatches = async (preferences) => {
    ...
    // Ensure midrashot data is loaded before calculating matches
    ...
    if (listToUse.length === 0) {
      ...
      try {
        ...
        console.error("Error fetching midrashot on finish:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="app-container">
      ...
      <header className="app-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.6rem', padding: '0 0.1rem' }}>
        ...
        >
          <Compass style={{ width: 22, height: 22, color: '#881337' }} />
          <span>מדרשון</span>
        </h1>
        ...
        {loading && view !== 'questionnaire' ? (
          <div className="glass-card" style={{ textAlign: 'center', padding: '3rem' }}>
            <h3 style={{ fontSize: '1.3rem', color: '#111827', marginBottom: '0.5rem' }}>
              טוען נתוני מדרשות מהמאגר...
            </h3>
            <p style={{ color: '#4b5563', fontSize: '0.9rem' }}>אנא המתיני מספר שניות</p>
          </div>
        ) : (
          <>
            {view === 'questionnaire' && (
      ...
      <footer style={{ marginTop: '2rem', padding: '1rem 0', textAlign: 'center', borderTop: '1px solid #f3c2ce' }}>
        ...
        <div style={{ fontSize: '0.78rem', color: '#64748b' }}>
          © מדרשון - מערכת להתאמת מדרשות לבנות
        </div>
      </footer>

      {/* Midrasha Request Modal */}
      <YeshivaRequestModal
