...
export default function ResultsView({ results, userPreferences, yeshivotList, onRestart, onRequestAddYeshiva }) {
  ...
  const sendAutomaticEmailToAdmin = async (requestData) => {
    ...
    const emailPayload = {
      to: adminEmail,
      subject: `[מדרשון] בקשה להוספת מדרשה חדשה (מתלמידה כיום): ${requestData.yeshiva_name}`,
      message: `התקבלה בקשה חדשה להוספת מדרשה למערכת ע"י תלמידה כיום:\n\n` +
               `שם המוסד: ${requestData.yeshiva_name}\n` +
               `מסלול: ${TYPE_TRANSLATIONS[requestData.type] || requestData.type}\n` +
               `אזור: ${REGION_TRANSLATIONS[requestData.region] || requestData.region}\n\n` +
               `פרמטרים מוצעים:\n${paramsList}\n\n` +
               `הערות: הוגש ע"י תלמידה כיום בשאלון מדרשון\n\n` +
    ...
    try {
      ...
      if (!existingYeshiva || selectedYeshivaName === 'other') {
        ...
        const requestPayload = {
          ...
          submitter_email: '',
          notes: 'הוגש ע"י תלמידה כיום בשאלון מדרשון'
        };
  ...
  return (
    <div className="results-container">
      {/* Top Match Banner */}
      <div className="glass-card" style={{ textAlign: 'center', borderColor: '#f3c2ce', background: '#fff0f3' }}>
        <div style={{ display: 'inline-flex', padding: '0.8rem', borderRadius: '50%', background: '#ffe4e6', color: '#be123c', marginBottom: '1rem' }}>
          <Award style={{ width: 44, height: 44 }} />
        </div>
        <h1 style={{ fontSize: '2.2rem', fontWeight: 900, marginBottom: '0.5rem', color: '#881337' }}>
          תוצאות מבחן ההתאמה במדרשון
        </h1>
        <p style={{ color: '#9f1239', fontSize: '1.1rem', fontWeight: 600 }}>
          המדרשה בעלת ההתאמה הגבוהה ביותר עבורך היא: <strong style={{ color: '#881337', fontSize: '1.4rem' }}>{topMatch?.name}</strong> עם {topMatch?.matchScore}% התאמה!
        </p>
      </div>
      ...
      <div className="glass-card">
        <h2 className="section-title">
          <CheckCircle className="w-5 h-5 text-rose-700" />
          המדרשות שהכי מתאימות לך:
        </h2>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.8rem' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.4rem' }}>
                    <span style={{ fontWeight: 900, color: '#881337', fontSize: '1.2rem' }}>#{index + 1}</span>
                    <h3 style={{ fontSize: '1.35rem', fontWeight: 800, margin: 0, color: '#111827' }}>{item.name}</h3>
                  </div>
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    <span className="tag">מסלול: {typeHebrew}</span>
      ...
      <div className="glass-card" style={{ borderColor: '#f3c2ce', background: '#fff0f3' }}>
        <h2 className="section-title" style={{ color: '#111827' }}>
          <HelpCircle className="w-6 h-6 text-rose-700" />
          לומדת כרגע במדרשה? עזרי לנו לשפר ולדייק את הנתונים!
        </h2>
        <p style={{ color: '#4b5563', marginBottom: '1.2rem', lineHeight: 1.6, fontWeight: 500 }}>
          כדי ששאלון מדרשון יהיה המדויק ביותר עבור השמיניסטיות הבאות, אנו אוספים ושומרים במאגר אך ורק תשובות של משתתפות הלומדות כיום במדרשות.
        </p>
        ...
        {submissionSaved ? (
            ...
            {isNewYeshivaRequest ? (
              <div style={{ background: '#ecfdf5', border: '1px solid #a7f3d0', padding: '1.2rem', borderRadius: 12, color: '#047857', textAlign: 'center' }}>
                <h3 style={{ fontSize: '1.2rem', fontWeight: 800, marginBottom: '0.4rem' }}>
                  ✓ הבקשה להוספת מדרשה חדשה נרשמה ונשלחה לאדמין!
                </h3>
                <p style={{ color: '#065f46', fontSize: '0.92rem', marginBottom: '0.8rem' }}>
                  תודה רבה! הבקשה להוספת המדרשה שלך נשמרה במאגר ונשלחה במייל לאדמין לאישור.
                </p>
              ...
              <div style={{ background: '#ecfdf5', border: '1px solid #a7f3d0', padding: '1.2rem', borderRadius: 12, color: '#047857', textAlign: 'center' }}>
                <h3 style={{ fontSize: '1.2rem', fontWeight: 800, marginBottom: '0.4rem' }}>
                  ✓ הדיווח שלך נרשם ונשמר במאגר בהצלחה!
                </h3>
                <p style={{ color: '#065f46', fontSize: '0.92rem' }}>
                  תודה רבה על תרומתך לדיוק הנתונים במדרשון! המערכת זוכרת ששלחת כבר את תשובותייך.
                </p>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.2rem' }}>
              ...
              >
                לא (אני שמיניסטית / אורחת)
              </button>
            </div>

            {isCurrentStudent === false && (
              <div style={{ background: '#ffe4e6', padding: '1rem', borderRadius: 8, color: '#374151', fontSize: '0.9rem', fontWeight: 500 }}>
                תודה! מכיוון שאינך לומדת כרגע במדרשה, תשובותייך לא ישמרו ב-DB כדי להבטיח שאך ורק דיווחים של בנות מדרשה ותלמידות יעדכנו את הנתונים המפוקחים.
              </div>
            )}

            {isCurrentStudent === true && (
              <form onSubmit={handleStudentSubmit} style={{ background: '#ffffff', padding: '1.25rem', borderRadius: 12, border: '1px solid #f3c2ce', animation: 'fadeIn 0.3s' }}>
                <label style={{ display: 'block', fontWeight: 700, fontSize: '1rem', marginBottom: '0.4rem', color: '#111827' }}>
                  באיזו מדרשה את לומדת כיום? *
                </label>
                ...
                <AutocompleteYeshivaSelect
                  ...
                  placeholder="הקלידי חיפוש שם מדרשה..."
                />
                ...
                {selectedYeshivaName === 'other' && (
                  <div style={{ marginBottom: '1rem' }}>
                    <input
                      ...
                      placeholder="הקלידי את שם המדרשה החדשה שלך..."
                ...
                {/* COMPACT SINGLE-PAGE 13 PARAMETERS EDITOR FOR CURRENT STUDENTS */}
                <div style={{ background: '#fff0f3', padding: '1rem', borderRadius: 12, marginBottom: '1.2rem', border: '1px solid #f3c2ce' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.95rem', fontWeight: 700, color: '#881337', marginBottom: '0.8rem' }}>
                    <Edit2 style={{ width: 16, height: 16 }} />
                    דייקי את 13 הפרמטרים עבור המדרשה שלך (הכול במסך אחד):
                  </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '1.2rem' }}>
                  ...
                  <label htmlFor="reflects_cb" style={{ cursor: 'pointer', fontSize: '0.95rem', color: '#111827', fontWeight: 500 }}>
                    הנתונים שהכנסתי כרגע בשאלון משקפים את המדרשה שלי וברצוני לתרום אותם לשיפור המערכת
                  </label>
                </div>
                ...
                >
                  ...
                  {isSubmitting ? 'שומר במאגר...' : 'שמרי תשובות במאגר'}
                </button>
              </form>
            )}
          </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', marginTop: '2rem' }}>
        ...
        <div style={{ textAlign: 'center', width: '100%', paddingTop: '1.5rem', borderTop: '1px solid #fbcfe8' }}>
          <div className="supporting-copy">לשאר המיזמים מבית "שבושון":</div>
          <div className="project-links">
            <a href="https://shvushon1.vercel.app" target="_blank" rel="noreferrer">שבושון</a>
          ...
          <div className="supporting-copy" style={{ fontSize: '0.9rem', color: '#4b5563', marginTop: '0.6rem' }}>
            <strong>יוצרות:</strong> אוריה קסנר, מעין רייך
          </div>
        </div>
      </div>
    </div>
